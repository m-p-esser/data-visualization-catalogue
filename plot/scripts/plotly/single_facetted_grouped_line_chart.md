...
x1, x2, y, hue, col = "MonthName", "MonthNumber", "Total Volume", "type", "region"
...
mean_table = df.groupby([x1, col, hue])[x2, y].mean() \
    .reset_index().sort_values(by=[col, x2, hue]) \
    .round(0)

# Draw Plot
fig = mean_table.pipe(
    lambda df: px.line(
        data_frame=df, x=x1, y=y, color=hue, template="seaborn",
